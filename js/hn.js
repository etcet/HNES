...
var CommentTracker = {
  ...
  process: function (data, request) {
    ...
    if (request.num) {
      info.num = request.num
    }

    var last_comment_id = info.last_comment_id
    if (request.last_comment_id) info.last_comment_id = request.last_comment_id
        ...
        HN.getLocalStorage(id, function (response) {
          if (response.data) {
            ...
            if (diff > 0) {
              ...
              el.empty()
              el.append(newcomm).append(totalcomm)
            }
          }
        })
      }
...
class HNComments {
  ...
  extractCommentParts(commentEl) {
    ...
    while (
      n &&
      !(n.nodeName == 'P' || n.nodeName == 'SPAN' || n.nodeName == 'DIV')
    ) {
  ...
  markupToNodeList(commentTree) {
    ...
    for (let i = 0; i < commentTables.length; i++) {
      const t = commentTables[i],
        ...
        isDead = t
          .querySelector('span.comhead')
          .textContent.includes(' [dead] '),
  ...
  nodeListToTree(nodeList) {
    const s = [],
      m = { root: nodeList[0] }
    for (
      let i = 0, j = 1, data = nodeList;
      j < data.length && data[j];
      i++, j++
    ) {
      const p = data[i],
        c = data[j]
  ...
  renderComment(c, into) {
    ...
    if (c.descCount > 0) {
      commentEl.querySelector('.reply-count').textContent = `(${
        c.descCount
      } repl${c.descCount == 1 ? 'y' : 'ies'})`
    }
    if (c.replyUrl) {
    ...
    for (
      let parts = c.textParts,
        textContainer = commentEl.querySelector('.text'),
        i = 0;
      i < parts.length;
      i++
    ) {
    ...
    commentEl.querySelector('.collapser').addEventListener(
      'click',
      e => {
        e.preventDefault()
        this.collapse(c)
    ...
    commentEl.querySelector('a.upvote').addEventListener(
      'click',
      e => {
        e.preventDefault()
    ...
    // ajax unvote
    commentEl.querySelector('a.unvote').addEventListener(
      'click',
      e => {
        e.preventDefault()
        var httpRequest = new XMLHttpRequest()
  ...
  apply() {
    ...
    if (!commentTree && !itemList && !threadList) {
      console.warn(
        'unrecognized markup detected, no commentTree, itemList, or threadList'
      )
      return
    } else if (itemList) {
...
var HN = {
  init: function () {
    ...
    if (postPagesRE.test(pathname)) {
    ...
    } else if (pathname == '/favorites' || pathname == '/upvoted') {
      $("td[colspan='2']").hide()
      $('.votelinks').hide()
      $('.ind').hide()
      //HN.doCommentsList(pathname, track_comments);
      ...
      HN.doUserProfile()
    } else if (pathname == '/newslogin' || pathname == '/login') {
      HN.doLogin()
  ...
  initElements: function () {
    ...
    var contentIndex = 2
    if (
      $('body > center > table > tbody > tr').eq(1).has('.pagetop').length > 0
    ) {
  ...
  doLogin: function () {
    ...
    $('tr#content').wrap(
      '<table border="0" cellpadding="0" cellspacing="0" width="85%"></table>'
    )

    // add header table row and submit button row
    $('tr#content').before(headerHtml)
    $('#login-form tr:last').after(
      '<tr><td></td><td>' + buttonHtml + '</td></tr>'
    )
    ...
    if (message)
      $('tr#content > td:first > h1').before(
        ' <p id="login-msg">' + message + '</p>'
      )
  ...
  doCommentsList: function (pathname, track_comments) {
    ...
    $("<p id='loading_comments'>Loading comments</p>").insertBefore(
      below_header[1]
    )
  ...
  doUserProfile: function () {
    ...
    if (options.length === 4) {
      //other user pages
      $('#user-profile a[href^="submitted"]')
        .parent()
        .attr('id', 'others-profile-submitted')
      about.next().linkify()
    } else {
      //your user page
      ...
      $('#user-profile a[href="changepw"]')
        .parent()
        .attr('id', 'your-profile-change-password')
      ...
      if (current_karma < karma_for_flag) {
        can_flag_msg = $(
          '<p>You need ' +
            (karma_for_flag - current_karma) +
            ' more karma until you can flag posts.</p>'
        )
      } else {
        can_flag_msg = $('<p>You can flag posts.</p>')
      }
      if (current_karma < karma_for_polls) {
        can_create_polls_msg = $(
          '<p>You need ' +
            (karma_for_polls - current_karma) +
            ' more karma until you can create a poll.</p>'
        )
      } else {
        can_create_polls_msg = $(
          '<p>You can <a href="//news.ycombinator.com/newpoll">create a poll</a>.</p>'
        )
      }
      if (current_karma < karma_for_downvotes) {
        can_downvote_msg = $(
          '<p>You need ' +
            (karma_for_downvotes - current_karma) +
            ' more karma until you can downvote comments.</p>'
        )
      } else {
      ...
      karma
        .next()
        .append(can_flag_msg)
        .append(can_create_polls_msg)
        .append(can_downvote_msg)
      ...
      var dead_explanation = $(
        '<p>Showdead allows you to see all the submissions and comments that have been killed by the editors.</p>'
      )
      showdead
        .next()
        .append($('<span>Default: no</span>'))
        .append(dead_explanation)

      var noprocrast_explanation = $(
        "<p>Noprocast is a way to prevent yourself from spending too much time on Hacker News. If you turn it on you'll only be allowed to visit the site for maxvisit minutes at a time, with gaps of minaway minutes in between.</p>"
      )
      noprocrast
        .next()
        .append($('<span>Default: no</span>'))
        .append(noprocrast_explanation)

      maxvisit.next().append($('<span>Default: 20</span>'))
      minaway.next().append($('<span>Default: 180</span>'))

      var delay_explanation = $(
        '<p>Delay allows you to delay the public posting of comments you make for delay minutes.</p>'
      )
      delay
        .next()
        .append($('<span>Default: 0</span>'))
        .append(delay_explanation)
    ...
    poll_scores.each(function () {
      ...
      if (score > 0) {
        ...
        var graph_el = $('<tr/>')
          .append($('<td/>'))
          .append(
            $('<td/>').append($('<div/>').addClass('poll-graph').width(width))
          )
  ...
  loadMoreLink: function (elem) {
    ...
    load_div.load(
      moreurl +
        ' > center > table > tbody > tr:nth-child(3) > td > table > tbody > tr',
      function (response) {
  ...
  replaceVoteButtons: function (isPostList) {
    ...
    $('img[src$="graydown.gif"]').replaceWith(
      '<div class="down-arrow last-arrow"></div>'
    )
  ...
  upvoteUserData: function (author, value) {
    // Adds value to the user's upvote count, saves and displays it.
    ...
    HN.getLocalStorage(author, function (response) {
      ...
      if (userInfo.votes) {
        // If we already have up/downvoted this user before.
  ...
  formatScore: function () {
    $('.subtext').each(function () {
      ...
      if (comments.text() == 'discuss' || /ago$/.test(comments.text())) {
        comments = $('<a/>').html('0').attr('href', comments.attr('href'))
      } else if (comments.text() == 'comments') {
        comments = $('<a/>').html('?').attr('href', comments.attr('href'))
  ...
  rewriteUserNav: function (pagetop) {
    ...
    var user_drop = $('<span/>')
      .append($('<a/>').text(user_name).attr('href', '#'))
    ...
    if (new_active) {
      if (
        window.location.pathname != '/upvoted' ||
      ...
      $('#top-navigation .nav-links').append(
        $('<span/>').text('|').append(new_active)
      )
    }

    hidden_div.append(logout.attr('id', 'user-logout').attr('title', 'Logout'))
  ...
  setTopColor: function () {
    var topcolor = document
      .getElementById('header')
      .children[0].getAttribute('bgcolor')
...
if (window.location.host == 'hckrnews.com') {
  $('ul.entries li').each(function () {
    chrome.runtime.sendMessage(
      { method: 'getLocalStorage', key: Number($(this).attr('id')) },
      function (response) {
        if (response.data != undefined) {
          ...
          var prepend =
            unread == 0 ? '' + unread + ' / ' : '<span>' + unread + '</span> / '
    ...
    if ('Unknown or expired link.' == $('body').html()) {
      ...
      HN.getLocalStorage('expired', function (response) {
        if (response.data != undefined) {
          ...
          if (expired) {
            $('#header').after(
              '<p id="alert">You reached an <a href="//news.ycombinator.com/item?id=17705" title="what?">expired page</a> and have been redirected back to the front page.</p>'
            )
...
;(() => {
  ...
  const comments = document.getElementById('hnes-comments').children

  /**
   *
   * @param {'previous' | 'next'} direction
   */
  function scrollToComment(direction) {
    let i = 0
    for (const comment of comments) {
      const isVisible = $(comment).visible()
      ...
      $('html,body').animate(
        {
          scrollTop: $(commentToScrollTo).offset().top,
        },
        'medium'
      )
